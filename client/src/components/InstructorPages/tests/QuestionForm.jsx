...
function QuestionForm({ onSave }) {
  ...
  return (
    ...
    >
      ...
      <Form.Item
        name="questionInfo"
        label={<span className="font-semibold">Question Content</span>}
        rules={[{ required: true }]}
      >
        <Input.TextArea />
      </Form.Item>
      <Form.Item
        name="marks"
        label={<span className="font-semibold">Marks</span>}
        rules={[{ required: true }]}
      >
        <InputNumber min={1} />
      </Form.Item>
      {(responseType === "single" || responseType === "multiple") && (
                ...
                >
                  ...
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "Correct indicator is required",
                      },
                    ]}
                    label="Correct"
                  >
                  ...
                  <MinusCircleOutlined
                    onClick={() => handleRemoveOption(index, fields, remove)}
                  />
                </Space>
              ))}
              <Button
      ...
      <Button
        type="primary"
        htmlType="submit"
        className="w-full main-black-btn mt-8"
      >
        Save Question
      </Button>
    </Form>
